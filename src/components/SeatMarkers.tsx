...
import { LocationCity, Place, Map, Info, CheckCircle, Cancel } from '@mui/icons-material';
...
export const SeatMarkers: React.FC<SeatMarkersProps> = ({
  seats
}) => {
  console.log('=== SeatMarkers renderizado ===');
  console.log('Número de asientos recibidos:', seats?.length || 0);

  if (!seats || seats.length === 0) {
    console.log('No hay asientos para mostrar');
    return null;
  }
  ...
  return (
    <>
      {seats.map((seat) => {
        ...
        return (
          <LeafletMarker
            key={seat.FID || `${seat.Asiento_Electoral}-${seat.Departamento}-${seat.Provincia}-${seat.Municipio}`}
            position={[lat, lng]}
                  ...
                  <Stack spacing={1.5}>
                      ...
                      <Box sx={{ mt: 0.5, pl: 2 }}>
                        <Stack direction="row" spacing={1} sx={{ mt: 0.5 }}>
                          <Chip 
                            ...
                            size="small"
                            sx={{ 
                              bgcolor: seat.Tipo_Urbano_Rural === 'Urbano' ? 'primary.50' : 'grey.100',
                              color: seat.Tipo_Urbano_Rural === 'Urbano' ? 'primary.dark' : 'text.secondary',
                              fontWeight: 600,
                              fontSize: '0.75rem',
                              height: 24
                            }}
                        ...
                        <Box sx={{ mt: 1, display: 'flex', alignItems: 'center', gap: 0.5 }}>
                          ...
                          <Typography 
                            variant="body2" 
                            color={seat.Estado === 'Habilitado TSE' ? 'success.main' : 'error.main'}
                            fontWeight="medium"
                            sx={{ fontSize: '0.85rem' }}
                          >
                            {seat.Estado}
                    ...
                    <Box>
                      ...
                      <Box sx={{ mt: 0.5, pl: 2 }}>
                        ...
                        >
                          {seat.Latitud?.toFixed(6)}, {seat.Longitud?.toFixed(6)}
                        </Typography>
                      </Box>
                    </Box>
                  </Stack>
